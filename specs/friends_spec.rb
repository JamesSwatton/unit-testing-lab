...
class TestFriends < MiniTest::Test
  ...
  def test_how_many_friends_after_adding_new_friend
    add_friend(@person4, "Scrappy-Doo")
    assert_equal(4, @person4[:friends].count)
  end

  # 5. For a given person, remove a specific name from their list of friends
  # (hint: Same as above, testing for the length of the array should be sufficient)
  def test_how_many_friends_after_removing_friend
    remove_friend(@person1, "Scooby")
    assert_equal(3, @person1[:friends].count)
  end

  # 6. Find the total of everyone's money
  # (hint: use the @people array, remember how we checked the total number of eggs yesterday?)
  def test_get_total_friends_money
    result = total_friends_monies(@people)
  ...
  def test_for_has_friend_lent_money_to_other_friend
    lend_money_to_friend(@person1, @person2, 1)
  ...
  def test_for_concatenating_friends_snack_arrays
    all_snacks = ["charcuterie", "soup", "bread", "Scooby snacks", "spaghetti", "ratatouille", "spinach"]
    result = add_friends_snacks_together(@people)
    assert_equal(all_snacks, result)
  end

  # 9. Find people with no friends
  # (hint: return an array, there might be more people in the future with no friends!)
  def test_for_no_friends
    no_mates = [@person5]
    result = friends_with_no_mates(@people)
    assert_equal(no_mates, result)
  end
end
